...
public class RazorAHRS {
	 ...
	 * thread.
	 *
	 * @param btDevice {@link BluetoothDevice BluetoothDevice} holding the Razor
	 * 		AHRS to connect to.
	...
	public RazorAHRS(BluetoothDevice btDevice, RazorListener razorListener)
		...
		this(btDevice, razorListener, RazorOutputMode.RAW_SENSOR_DATA);
	}
	...
	private void write(String text) throws IOException {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
			outStream.write(text.getBytes(StandardCharsets.US_ASCII));
		}
	}
	...
	private class BluetoothThread extends Thread {
		...
		private void initRazor() throws IOException {
			...
			String s = SYNCH_TOKEN + contactSynchID + NEW_LINE;
			// again, no idea if this will work
			byte[] contactSynchReply = new byte[0];
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
				contactSynchReply = s.getBytes(StandardCharsets.US_ASCII);
			}
			...
			s = SYNCH_TOKEN + configSynchID + NEW_LINE;
			//final byte[] configSynchReply = EncodingUtils.getAsciiBytes(SYNCH_TOKEN + configSynchID + NEW_LINE);
			final byte[] configSynchReply = s.getBytes(StandardCharsets.US_ASCII);
